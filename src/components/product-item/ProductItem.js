...
const WooProduct = ({ data, deleteFunc, openModalEdit, liveUrl }) => {
  ...
  return (
    <li className={`product-item ${data.isUpdated ? "product-item-updated" : ""}`} id={data.id}>
      <div className="product">
        <ModalConfirmation
          product={data.name}
          openModalConfirmation={showConfirmation}
          validateAction={(val) => deleteProduct(val)}
        />
        <div className="product-img-container">
          {data.images.length > 0 && image !== false ? (
            <div className="thumbnail" style={{ backgroundImage: `url(${image})` }}>
              {!isImageLoading && <div className="loading-animation"></div>}
              <img className="visual" src={image} alt="" onLoad={() => setIsImageLoading(true)} />
            </div>
          ) : (
        ...
        <div className="product-info">
          ...
          <ToggleDisplay
            show={Boolean(data.regular_price.length)}
            className={`details ${data.sale_price && data.regular_price !== data.sale_price ? "in-promo" : ""}`}
          >
            {data.sale_price && data.regular_price !== data.sale_price ? (
              <span className="price promo">${data.sale_price} </span>
            ) : null}
        ...
        >
          <ul className="popover-links">
            <li>
              <Link to={APP_PATHS.EDIT_PRODUCT.replace(":productId", data.id)}>Advanced Edit</Link>
            </li>
            <li>
