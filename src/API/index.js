...
// WooCommerce API
export const WCFM = `${APP_API_URL}/wp-json/wcfmmp/v1`;
// JWT API
export const AUTH = `${APP_API_URL}/wp-json/jwt-auth/v1`;

const LOGIN = (formData) => {
  return axios.post(`${AUTH}/token`, formData).then((tokenResult) => {
...
const WC_getWooProductByName = () => {
  ...
  return (token, productName) => {
    ...
    return axios
      .get(`${WC}/products?search=${productName}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const WC_getWooCategories = (token, hideempty = false) => {
  return axios
    .get(`${WC}/products/categories?per_page=100&hide_empty=${hideempty}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
const WC_getWooTags = (token) => {
  return axios
    .get(`${WC}/products/tags?per_page=100`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
const WC_createWooCategories = (token, data) => {
  let newTag = data;
  return axios
    .post(`${WC}/products/categories`, newTag, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
const WP_change_password = (token, payload) => {
  return axios
    .put(`${WP}/current-user/${payload.id}/change_password`, payload, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
const WC_deleteProduct = (token, id) => {
  return axios
    .delete(`${WC}/products/${id}?force=true`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
